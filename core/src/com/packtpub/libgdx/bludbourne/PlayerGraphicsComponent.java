...
public class PlayerGraphicsComponent extends GraphicsComponent {
        ...
        if(string.length == 2 ){
            if (string[0].equalsIgnoreCase(
                ...
                _currentPosition = _json.fromJson(Vector2.class, string[1]);
                    ...
                    MESSAAGE.CURRENT_STATE.toString())) {
                ...
                _currentDirection = _json.fromJson(
                        Entity.Direction.class, string[1]);
            }else if (string[0].equalsIgnoreCase(
                    MESSAGE.LOAD_ANIMATIONS.toString())) {
                EntityConfig entityConfig = _json.fromJson(
                        EntityConfig.class, string[1]);
                ...
                for(AnimationConfig animationConfig : animationConfigs) {
                    Array<String> textureNames =
                            animationConfig.getTexturePaths();
                    Array<Gridpoint2> points =
                            animationConfig.getGridPoints();
                    Entity.AnimationType animationType = animationConfig.getAnimationType();
                    float frameDuration = animationConfig.getFrameDuration();
                    Animation animation = null;

                    if (textureNames.size == 1) {
                        animation = loadAnimation(texture.get(0), points, frameDuration);
                    }else if (textureNames.size == 2){
                        animation = loadAnimation(textureNames.get(0),
                                textureNames.get(1), points, frameDuration);
                    }
    ...
    public void update(Entity entity, MapManager mapMgr, Batch batch, float delta){
        ...
        camera.position.set(_currentPosition.x, _currentPosition.y, 1, 1);
        batch end();
